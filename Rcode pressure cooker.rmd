...
file_path <- # e.g. "~/Documents/Pressure-Cooker-Group-2/sessionInfo"
...
# The files and sessions didn't have nice names, so we created file_ID and session_ID
full_data <- full_data %>%
...
hint_spam_summary <- full_data %>%
  filter(event == "HINT") %>%  # only consider sessions with at least one HINT
  group_by(session_id) %>%
  summarise(spammed_hint = any(spam_hint == 1, na.rm = TRUE))  # check if any spam occurred in session

# Proportion of sessions where hints were spammed (among sessions where hint used)
hint_spam_summary %>%
  summarise(
...
eval_spam_summary <- full_data %>%
  ...
  mutate(
    ...
    is_spam_eval = time_diff <= 3 & progress_diff <= 0  # define spam as quick repeat without improvement
  ) %>%
  summarise(
    num_evals = n(),
...
spam_eval_sessions <- full_data %>%
  ...
  summarise(spammed_eval = any(is_spam_eval, na.rm = TRUE), .groups = "drop") %>%
  filter(spammed_eval) %>%
  pull(session_id)

# Proportion of sessions where evaluate was spammed (among sessions where the first evaluate wasn't correct)
tibble(
...
hint_improvement <- filtered_data %>%
  ...
  mutate(progress_change = next_eval_progress - if_else(prev_event == "EVALUATE", prev_eval_progress, 0)) %>%
  ungroup()
